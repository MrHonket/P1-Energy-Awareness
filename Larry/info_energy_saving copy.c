...
{   
    ...
    for(int i=1; i<n_elements;i++){
        if(current_price > data_array[i].prize.DK1price){
            cheapest_price_index = i;
        }
    }
    cheapest_price = data_array[cheapest_price_index].prize.DK1price;
    ...
    // current_consumption = data_array[User.choice.hour].meter.value;
    current_price = data_array[0].prize.DK2price;
    current_consumption = data_array[0].meter.value;
    ...
    // qsort(data_array, n_elements, sizeof(data), cmpfunc_DK1);
    for(int i=1; i<n_elements;i++){
        if(current_price > data_array[i].prize.DK2price){
            cheapest_price_index = i;
        }
    }
    cheapest_price = data_array[cheapest_price_index].prize.DK2price;
...
void overview_for_interval(user *User, data data_array[])
{
    ...
    if (User->settings.residence == Dk1)
    {
        ...
        data_array->prize.from.year, area_txt[User->settings.residence]);
