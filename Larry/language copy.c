...
void l_prompt_user(user *User){
    ...
    else if(User->settings.language == ENG){
...
void cheapest(data data_array[], user *User)
{
    ...
    else if (User->settings.residence == Dk1){
...
void l_update_settings(user *User){
    ...
    if(User->settings.language == DK){
        ...
        // User->settings = load_settings(User);
    }
    else if(User->settings.language == ENG){
...
void init_user(user *User){
    User->type = NO_USER;
    User->settings.id = 100;
    User->settings.language = NO_LANG;
    User->settings.next_activation = date_from_stringDMYI("0-0-0" ,0); 
    User->settings.residence = NO_AREA;
    User->choice.function = UpdateSettings;
    User->choice.hour = 0;
    User->choice.lookup = NO_LOOKUP;
    User->choice.mean_or_median = NO_MEAN_MEDIAN;
