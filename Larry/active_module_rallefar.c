...
int main(void){
    user User = { {200, "DK1", "DK"}, InfoEnergySaving, Mean, {{13, 0}, 0, Januar, 2017}, {{14, 0}, 0, Januar, 2017}, Price, Human};       
...
int prompt_user(user User, data *Data, data cheapest_struct, double user_price){
    ...
    if(User.choice.function == UserHistory){
        ...
        printf("--------- SORTERET --------\n");
        for (int i = 0; i < 24; i++)
            printf("Dato: %d\tTidspunkt: %d - %d\tPris: %.2f\n", Data[i].prize.from.day, 
                                        Data[i].prize.from.time.hour, Data[i].prize.to.time.hour, Data[i].prize.DK1price);
